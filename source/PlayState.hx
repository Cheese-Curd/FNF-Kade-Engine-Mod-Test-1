...
class PlayState extends MusicBeatState
{
	...
	// Annie Week Stuff \\
	var annieTrafficLights:FlxTypedGroup<FlxSprite>;
	var anniecar:FlxSprite;
	...
	override public function create()
	{
		...
		switch (songLowercase)
		{
			...
			case 'wocky':
				dialogue = CoolUtil.coolTextFile(Paths.txt('data/wocky/wockyDialogue'));
			case 'beathoven':
				dialogue = CoolUtil.coolTextFile(Paths.txt('data/beathoven/beathovenDialogue'));
			case 'hairball':
				dialogue = CoolUtil.coolTextFile(Paths.txt('data/hairball/hairballDialogue'));
			case 'nyaw':
				dialogue = CoolUtil.coolTextFile(Paths.txt('data/nyaw/nyawDialogue'));
		}
				...
				case 'annie':
					{
						...
						var bg:FlxSprite = new FlxSprite(-100).loadGraphic(Paths.image('annie/sky', 'week8'));
						bg.scrollFactor.set(0.1, 0.1);
						add(bg);

						var annieCity:FlxSprite = new FlxSprite(-10).loadGraphic(Paths.image('annie/city', 'week8'));
						annieCity.scrollFactor.set(0.3, 0.3);
						annieCity.setGraphicSize(Std.int(annieCity.width * 0.85));
						annieCity.updateHitbox();
						add(annieCity);

						annieTrafficLights = new FlxTypedGroup<FlxSprite>();
						if (FlxG.save.data.distractions)
						{
							add(annieTrafficLights);
						}

						for (i in 0...5)
						{
							var annieLight:FlxSprite = new FlxSprite(annieCity.x).loadGraphic(Paths.image('annie/win' + i, 'week8'));
							annieLight.scrollFactor.set(0.3, 0.3);
							annieLight.visible = false;
							annieLight.setGraphicSize(Std.int(annieLight.width * 0.85));
							annieLight.updateHitbox();
							if(FlxG.save.data.antialiasing)
								{
									annieLight.antialiasing = true;
								}
							annieTrafficLights.add(annieLight);
						}

						var annieStreet:FlxSprite = new FlxSprite(-40, 50).loadGraphic(Paths.image('annie/behindTrain', 'week8'));
						add(annieStreet);

						anniecar = new FlxSprite(2000, 360).loadGraphic(Paths.image('annie/train', 'week8'));
						if (FlxG.save.data.distractions)
						{
							add(anniecar);
						}

						carSound = new FlxSound().loadEmbedded(Paths.sound('car_passes', 'week8'));
						FlxG.sound.list.add(carSound);

						// var cityLights:FlxSprite = new FlxSprite().loadGraphic(AssetPaths.win0.png);

						var annieStreet2:FlxSprite = new FlxSprite(-40, annieStreet.y).loadGraphic(Paths.image('annie/street', 'week8'));
						add(annieStreet2);
					}
				case 'limo':
					{
		...
		kadeEngineWatermark = new FlxText(4, healthBarBG.y
			...
			+ (Main.watermarks ? " | stop playing please I beg of you" : ""), 16);
	...
	override public function update(elapsed:Float)
	{
		...
		switch (curStage)
		{
			...
			case 'annie':
				if (carMoving && !PlayStateChangeables.Optimize)
				{
					carFrameTiming += elapsed;

					if (carFrameTiming >= 1 / 24)
					{
						updateCarPos();
						carFrameTiming = 0;
					}
	...
	// Annie Background shit \\
	var carMoving:Bool = false;
	var carFrameTiming:Float = 0;

	var Cars:Int = 8;
	var carFinishing:Bool = false;
	var carCooldown:Int = 0;

	function carStart():Void
	{
		if (FlxG.save.data.distractions)
		{
			carMoving = true;
	...
	var carStartedMoving:Bool = false;

	function updateCarPos():Void
	{
		if (FlxG.save.data.distractions)
		{
			if (carSound.time >= 4700)
			{
				carStartedMoving = true;
				gf.playAnim('hairBlow');
			}
			...
			if (carStartedMoving)
			{
				...
				if (anniecar.x < -2000 && !carFinishing)
				{
					anniecar.x = -1150;
					Cars -= 1;
				...
				if (anniecar.x < -4000 && carFinishing)
					carReset();
			}
		}
	}

	function carReset():Void
	{
		if (FlxG.save.data.distractions)
		{
			gf.playAnim('hairFall');
			anniecar.x = FlxG.width + 200;
			carMoving = false;
			// trainSound.stop();
			// trainSound.time = 0;
			Cars = 8;
		...
		switch (curStage)
		{
			...
			case "annie":
				if (FlxG.save.data.distractions)
				{
					...
					if (curBeat % 4 == 0)
					{
						annieTrafficLights.forEach(function(light:FlxSprite)
						{
							light.visible = false;
						});

						curLight = FlxG.random.int(0, annieTrafficLights.length - 1);

						annieTrafficLights.members[curLight].visible = true;
						// phillyCityLights.members[curLight].alpha = 1;
					}
				}

				if (curBeat % 8 == 4 && FlxG.random.bool(30) && !carMoving && carCooldown > 8)
				{
					if (FlxG.save.data.distractions)
					{
						carCooldown = FlxG.random.int(-4, 0);
						carStart();
					}
				}
		}
